...
describe("buildScale", () => {
  it("places the marker proportionally (higher-is-better)", () => {
    expect(buildScale({ value: 75, min: 0, max: 100, format: fmt }).position).toBeCloseTo(
      0.75,
      5,
    );
  });
  ...
  it("flips the track when lower-is-better", () => {
    const lo = buildScale({ value: 10, min: 0, max: 100, higherIsBetter: false, format: fmt });
    const hi = buildScale({ value: 90, min: 0, max: 100, higherIsBetter: false, format: fmt });
    expect(lo.position).toBeGreaterThan(hi.position);
  });

  it("labels the axis ends in reading order (worst → best)", () => {
    const s = buildScale({ value: 5, min: 1, max: 9, higherIsBetter: false, format: fmt });
    expect(s.minLabel).toBe("9");
    expect(s.maxLabel).toBe("1");
  });

  it("clamps out-of-domain values", () => {
    expect(buildScale({ value: 999, min: 0, max: 100, format: fmt }).position).toBe(1);
    expect(buildScale({ value: -5, min: 0, max: 100, format: fmt }).position).toBe(0);
  });
  ...
  it("keeps the log-scaled marker inside the track", () => {
    const s = buildScale({ value: 20, min: 5, max: 250, log: true, format: fmt });
